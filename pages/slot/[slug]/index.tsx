...
const SlotPage: FunctionComponent<PageProps> = ({ data, article }) => {
	...
	const { removeLikeSlotId, setRemoveLikeSlotId } = useContext(
		removeLikeSlotContext
	)
	...
	const handleKeyDown = (event: KeyboardEvent) => {
		if (event.code === 'Escape') document.exitFullscreen()
	}
	...
	useEffect(() => {
		...
		if (
			currentItem &&
			JSON.parse(currentItem).some((slot: Slot) => slot.id === data.id)
		)
			setLikedSlot(true)
		...
		if (currentItem) {
			if (likedSlot) {
				...
				localStorage.setItem(
					Category.FAVORITES,
					JSON.stringify(currFavorites)
				)
			} else {
				const newItems: any[] = JSON.parse(currentItem).filter(
					(card: Slot) => card.id !== data.id
				)
		...
		<Layout title={`Slot Machine - ${data.name}`}>
			<Fragment>
				<Main>
					<GameContainer>
						...
						>
							{showIframe && (
								<Iframe
									...
									allow={'fullscreen'}
									sandbox={
										'allow-orientation-lock allow-scripts allow-same-origin'
									}
									scrolling={'no'}
								/>
							)}
							...
							<Actions className={'slot-actions'}>
								...
								<LikeButton
									setActive={() => setLikedSlot(!likedSlot)}
									active={likedSlot}
								/>
							</Actions>
						</IframeContainer>
						...
						<Section>
											...
											<td>
												<div>
													<span className='label'>
														...
														<Tooltip
															title={
																"L'RTP indica il ritorno al giocatore in percentuale che il gioco eroga sottoforma di pagamenti.'"
															}
															classes={{
																tooltip:
																	classes.tooltip,
												...
												{data?.volatility && (
													<div>
														<span className='label'>
															...
															<Tooltip
																title={
																	'La volatilità indica la frequenza con cui un gioco eroga vincita. Bassa = vincite frequenti ma basse. Alta = vincite rare ma di moltiplicatore elevato.'
																}
																classes={{
																	tooltip:
																		classes.tooltip,
												...
												{data?.winningSpinFrequency && (
													<div>
														<span className='label'>
															...
															<Tooltip
																title={
																	'Questo numero da 1 a 5 indica la frequenza di spin in cui si ottengono vincite. Più è basso e più è probabile che molti spin vadano a vuoto.'
																}
																classes={{
																	tooltip:
																		classes.tooltip,
												...
												{data?.maxLines && (
													<div>
														<span className='label'>
															...
															<Tooltip
																title={
																	'Il numero massimo di linee di pagamento utilizzabili nel gioco.'
																}
																classes={{
																	tooltip:
																		classes.tooltip,
							...
							>
								{mainBonus && (
									<MainBonusCard data={mainBonus} />
								)}

								<RelatedBonuses>
									{data.bonuses &&
										data.bonuses.map(
											(bonus: Bonus, index: number) => (
												<FreeBonusCard
													key={index}
													data={bonus}
												/>
				...
				<div className='layout-container'>
					...
					<SlotReview data={data} article={article} />
					<br />
				</div>
			</Fragment>
		</Layout>
...
export async function getServerSideProps(context: NextPageContext) {
	...
	const additionalData = await aquaClient.query({
		query: `query($slug:String="piggy-riches") {
                            casinoSquadSlotArticles(where:{slug:$slug}){
                                article
                                slug
                                seoTitle
                                seoDescription
	...
	if (additionalData.data.data.casinoSquadSlotArticles.length > 0)
		article = additionalData.data.data.casinoSquadSlotArticles[0]
	else article = null
	return {
		props: {
...
const IframeContainer = styled.div<ThumbnailProps>`
	...
	.fullscreen {
		...
		transform: ${({ isFullscreen }) =>
			isFullscreen ? 'translate(-50%, -50%)' : 'unset'};
	}
